"""Report assembly (Markdown + JSON) and Discord delivery for the TW team."""
...
def build_markdown(result: dict) -> str:
    ...
    lines = [
        f"# 台股 AI 投資團隊報告 — {result['generated_at']}",
        ...
        "",
        "## 投資組合",
        f"- 權益 **NT${pf['equity']:,.2f}**｜現金 NT${pf['cash']:,.2f} ({pf['cash_pct']:.1f}%)｜曝險 {pf['gross_exposure_pct']:.1f}%",
        f"- 累計報酬 **{pf['total_return_pct']:+.2f}%**｜回撤 {pf['drawdown_pct']:.1f}%｜已實現 NT${pf['realized_pnl']:,.2f}",
    ]
    if pf["positions"]:
        lines.append("")
        lines.append("| 標的 | 張數 | 成本 | 現價 | 權重 | 損益 | 停損 |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|")
        for r in pf["positions"]:
            lines.append(f"| {r['symbol']} | {r['lots']:g} | {r['avg_cost']:.2f} | {r['price']:.2f} | "
    ...
    if result.get("skipped_stops"):
        lines += ["", "## 停損失效警示（跌停鎖死，無法成交）"]
        for s in result["skipped_stops"]:
            lines.append(f"- {s}：停損延續至下一輪")
...
def save_report(result: dict, report_dir: Path | None = None) -> Path:
    ...
    md_path = report_dir / f"tw_team_{ts}.md"
    md_path.write_text(build_markdown(result), encoding="utf-8")
    json_path = report_dir / f"tw_team_{ts}.json"
...
def send_discord(result: dict) -> bool:
    ...
    fields = [
        {"name": "💰 權益", "value": f"`NT${pf['equity']:,.0f}` ({pf['total_return_pct']:+.1f}%)", "inline": True},
    ...
    if fills:
        fields.append({"name": "✅ 本輪成交", "inline": False,
                       "value": "\n".join(f"`{f['side']} {f['symbol']} {f['shares']/1000:g}張 @ {f['price']:.2f}`" for f in fills)[:1000]})
    if pf["positions"]:
    ...
    for chunk in _split_message(detail):
        _post({"content": chunk})
        time.sleep(0.8)
    return True
...
def send_startup(interval_minutes: int, market_open_only: bool) -> bool:
    return _post({"embeds": [{
        "title": "🚀 台股 AI 投資團隊已啟動",
        "description": f"每 **{interval_minutes} 分鐘**執行一次。" + ("僅在台股交易時段執行。" if market_open_only else ""),
        "color": COLOR_BLUE,
        "fields": [
            {"name": "團隊", "value": "宏觀策略師・基本面・技術面・籌碼面・催化劑・多方・空方・風險長・PM", "inline": False},
            {"name": "目標", "value": "NT$1,000,000 → NT$3,000,000 / 3 年（模擬帳戶）", "inline": False},
        ],
    }]})
