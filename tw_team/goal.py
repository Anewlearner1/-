"""Goal math for the NT$1M → NT$3M in 3 years mandate.
...
def describe_goal(status: GoalStatus) -> str:
    ...
    lines = [
        f"- 起始資金: NT${status.start_capital:,.0f} → 目標: NT${status.target_capital:,.0f} "
        f"({status.multiple_required_total:.1f}x)，期限 {status.horizon_years:.0f} 年，起算日 {status.start_date}",
        f"- 目前權益: NT${status.current_equity:,.2f}｜已經過 {status.elapsed_years:.2f} 年｜剩餘 {status.remaining_years:.2f} 年",
        ...
        f"- 等速路徑今日應有權益: NT${status.expected_equity_on_path:,.0f}｜進度比 {status.on_track_ratio:.2f}｜狀態: {status.status}",
