"""Orchestrator: one full team cycle.
    ...
    data (incl. 籌碼) → stop-loss sweep → 5 analysts (parallel) → bull → bear
...
def _limit_flags(packet: MarketPacket) -> tuple[set[str], set[str]]:
    up, down = set(), set()
    for sym, row in packet.stocks.items():
        if row.get("limit_up"):
            up.add(sym)
...
def run_team(no_trade: bool = False, packet: MarketPacket | None = None,
    ...
    limit_up, limit_down = _limit_flags(packet)

    stop_fills = []
    if not no_trade:
        stop_fills = pf.check_stops(prices, limit_down=limit_down, when=now)
        for t in stop_fills:
            print(f"  [停損] 賣出 {t.symbol} {t.shares:g} 股 @ {t.price:.2f}，實現 {t.realized_pnl:+.2f}")
    skipped_stops = [s for s in pf.positions if s in limit_down and pf.positions[s].stop_loss
                     and prices.get(s, float("inf")) <= pf.positions[s].stop_loss]
    for s in skipped_stops:
        print(f"  [停損失效] {s} 觸及跌停鎖死，停損無法成交，停損延續至下一輪")
    ...
    print(f"  [組合] 權益 NT${equity_before:,.2f}｜回撤 {pf_summary['drawdown_pct']:.1f}%｜目標狀態 {goal.status}")

    # ---------------- 2. Analysts in parallel ----------------
    print(f"  [團隊] 五位分析師並行分析中 ({llm.model})...")
