"""
Taiwan Stock AI Investment Team — scheduler entry point.

Usage:
    python run_tw_team.py --once              # one full team cycle, then exit
    python run_tw_team.py --once --no-trade   # analyse and decide, but do not touch the paper portfolio
    python run_tw_team.py --status            # print portfolio + goal progress (no LLM calls)
    python run_tw_team.py --interval 240      # run every 4 hours
    python run_tw_team.py --reset             # start a fresh paper portfolio (asks for confirmation)

Environment (see .env.example):
    ANTHROPIC_API_KEY      required (or `ant auth login`)
    DISCORD_WEBHOOK_URL    optional push notifications
    TW_TEAM_*              model / effort / web search / paths
    RISK_* / TW_*           hard risk limits + trading cost model
    GOAL_*                 goal parameters
    MARKET_OPEN_ONLY=1     only run during Taiwan market hours (Mon-Fri 09:00-13:30 CST)
"""
...
from tw_team.config import GOAL, RUNTIME
from tw_team.data import fetch_history
from tw_team.goal import describe_goal, evaluate_goal, milestones
from tw_team.llm import api_key_present
from tw_team.portfolio import describe_portfolio
from tw_team.report import save_report, send_discord, send_error, send_startup
from tw_team.team import load_portfolio, portfolio_path, run_team
...
def show_status() -> None:
    ...
    for m in milestones(GOAL, pf.start_date):
        print(f"  第 {m['month']:2d} 個月: NT${m['equity_target']:,.0f}")
...
def reset_portfolio() -> None:
    ...
    if path.exists():
        ...
        path.unlink()
    pf = load_portfolio()
    print(f"已建立新模擬帳戶：現金 NT${pf.cash:,.2f}，起算日 {pf.start_date}")


def run_once(no_trade: bool) -> dict | None:
    if RUNTIME.market_open_only and not is_market_hours():
        print(f"  [跳過] 目前 {datetime.now(TAIPEI_TZ):%H:%M} 非台股交易時段 (09:00-13:30，週一至週五)")
        return None
    try:
...
def run_scheduler(interval_seconds: int, no_trade: bool) -> None:
    ...
    print(f"\n  台股 AI 投資團隊啟動｜每 {interval_seconds // 60} 分鐘｜報告目錄 {RUNTIME.report_dir.resolve()}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="台股 AI 投資團隊 — 多角色 Claude 決策 + 模擬帳戶")
